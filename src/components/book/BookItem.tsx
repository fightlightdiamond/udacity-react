    ...
    return (
        ...
        >
            ...
            <Dropdown placement="top" size={'xs'} label="Move to...">
                {categories.map((cate) => cate === book.category ?
                    <Dropdown.Item key={book.id} onClick={() => onChangeCategory(book.id, cate)}
                                   icon={HiCheckCircle}>{cate}</Dropdown.Item>
                    : <Dropdown.Item onClick={() => onChangeCategory(book.id, cate)}>{cate}</Dropdown.Item>
                )}
            </Dropdown>
        </Card>
    );
